...
/**
 * @title LuxfiToken
 * @notice LUXFI Web 4.0 Token — Production hardened
 * @dev Security: AccessControl roles, Chainlink staleness, slippage protection,
 *      authorized burners, governance notification, circular referral guard,
 *      recoverERC20 blocked for LUXFI itself
 */
contract LuxfiToken is ERC20, AccessControl, Pausable, ReentrancyGuard {
    ...
    // ─── REFERRAL — FIX circular referral ─────────────────
    ...
    function purchaseWithToken(
        ...
        _processBNBPurchase(
            msg.sender, bnbReceived,
            purchaseFeeBps + paymentTokens[token].conversionFeeBps,
            token, referrer
        );
    }
    ...
    function _processBNBPurchase(
        ...
        // FIX — circular referral guard
        if (
            referrer != address(0) &&
            referrer != buyer &&
            referredBy[buyer] == address(0) &&
            referredBy[referrer] != buyer  // prevent A→B→A
        ) {
        ...
        require(
            balanceOf(buyer) + tokenAmount <= (MAX_SUPPLY * MAX_WALLET_PERCENT) / PERCENT_DENOMINATOR,
            "Exceeds max wallet"
        );
        ...
        if (ecoGovernor != address(0)) {
            try IGovernanceAcquisition(ecoGovernor).recordTokenAcquisition(buyer) {} catch {}
        }
        if (brandGovernor != address(0)) {
            try IGovernanceAcquisition(brandGovernor).recordTokenAcquisition(buyer) {} catch {}
        }
    ...
    function burn(uint256 amount) external {
        require(authorizedBurners[msg.sender] || hasRole(ADMIN_ROLE, msg.sender), "Not authorized to burn");
    ...
    function recordTokenAcquisition(address account) external {
        require(hasRole(OPERATOR_ROLE, msg.sender) || isExcludedFromFee[msg.sender], "Not authorized");
        if (lastTransferTime[account] == 0) lastTransferTime[account] = block.timestamp;
    }

    // ─── FIX — recoverERC20 blocked for LUXFI itself ──────
    function recoverERC20(address token, uint256 amount) external onlyRole(ADMIN_ROLE) {
        require(token != address(this), "Cannot recover LUXFI token");
    ...
    function setAuthorizedBurner(address burner, bool status) external onlyRole(ADMIN_ROLE) {
        authorizedBurners[burner] = status;
        emit BurnerUpdated(burner, status);
    }

    function setPurchaseFee(uint256 feeBps) external onlyRole(OPERATOR_ROLE) {
        require(feeBps <= MAX_FEE_BPS, "Fee too high");
        purchaseFeeBps = feeBps;
        emit FeeUpdated("PURCHASE", feeBps);
    }

    function setSellFee(uint256 feeBps) external onlyRole(OPERATOR_ROLE) {
        require(feeBps <= MAX_FEE_BPS, "Fee too high");
        sellFeeBps = feeBps;
        emit FeeUpdated("SELL", feeBps);
    }

    function setTransferFee(uint256 feeBps) external onlyRole(OPERATOR_ROLE) {
        require(feeBps <= 100, "Max 1% transfer fee");
        transferFeeBps = feeBps;
        emit FeeUpdated("TRANSFER", feeBps);
    }

    function setReferralFee(uint256 feeBps) external onlyRole(OPERATOR_ROLE) {
        require(feeBps <= 200, "Max 2% referral fee");
        referralFeeBps = feeBps;
    }

    function setMaxSlippage(uint256 slippageBps) external onlyRole(OPERATOR_ROLE) {
        require(slippageBps <= MAX_ALLOWED_SLIPPAGE_BPS, "Max 5%");
        require(slippageBps >= 10, "Min 0.1%");
        maxSlippageBps = slippageBps;
        emit SlippageUpdated(slippageBps);
    }

    function setPaymentToken(
        address token, bool accepted, uint256 conversionFeeBps, string calldata symbol
    ) external onlyRole(OPERATOR_ROLE) {
        require(token != address(0), "Invalid token");
        require(conversionFeeBps <= 200, "Max 2%");
    ...
    function setFeeDistributor(address distributor) external onlyRole(ADMIN_ROLE) {
    ...
    function setBlacklisted(address account, bool status) external onlyRole(OPERATOR_ROLE) {
        blacklisted[account] = status;
        emit Blacklisted(account, status);
    }

    function setExcludedFromFee(address account, bool excluded) external onlyRole(OPERATOR_ROLE) {
        isExcludedFromFee[account] = excluded;
    }

    function setPurchaseCooldown(uint256 cooldown) external onlyRole(OPERATOR_ROLE) {
        require(cooldown <= 1 hours, "Too long");
